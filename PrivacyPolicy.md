# Privacy Policy

**Last Updated: May 6, 2026**

## 1. Introduction

Vowed ("we," "us," or "our") respects your privacy and is committed to protecting your personal information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our public commitment platform.

## 2. Information We Collect

### 2.1 Information You Provide

**Account Information:**

- Name, email address, and profile information from authentication providers
- Username and profile preferences

**Content Information:**

- Vow titles, descriptions, deadlines, and visibility settings
- Comments on vows
- Resolution notes and outcomes

**Communications:**

- Messages sent through our support channels
- Feedback and bug reports

### 2.2 Information We Collect Automatically

**Usage Data:**

- Vow view counts and engagement metrics (collected for service functionality)
- Basic server logs including IP addresses and device information (used for security and troubleshooting)

**Cookies and Tracking:**

- Session cookies for authentication (provided by Clerk)
- Preference cookies for theme settings

### 2.3 Information from Third Parties

**Authentication Providers:**

- We use Clerk for user authentication
- We receive basic profile information from authentication providers

## 3. How We Use Your Information

### 3.1 Service Provision

- To create and manage your account
- To display your vows and profile information
- To enable commenting and social features
- To provide customer support

### 3.2 Service Improvement

- To develop new features and services based on user feedback
- To troubleshoot technical issues

### 3.3 Communication

- To send service-related notifications
- To respond to your inquiries
- To provide important updates about the service

### 3.4 Legal Compliance

- To comply with legal obligations
- To enforce our terms of service
- To protect the rights and safety of users

## 4. Information Sharing and Disclosure

### 4.1 Public Information

- Vow content is public by default unless marked as private
- Public vows are visible to all users and search engines
- User profiles and public vow lists are accessible to other users

### 4.2 Service Providers

- We share information with trusted third-party service providers who assist in operating our service
- These providers include hosting services, authentication providers, and analytics tools
- We require these providers to maintain appropriate security measures

### 4.3 Legal Requirements

- We may disclose information if required by law
- We may disclose information to protect our rights or the safety of users
- We may disclose information in connection with legal proceedings

### 4.4 Business Transfers

- In the event of a merger, acquisition, or sale of assets, user information may be transferred
- We will notify users of any such transfer

## 5. Data Retention

### 5.1 User Accounts

- We retain account information while your account is active
- We may retain information after account deletion for legal, regulatory, or legitimate business purposes

### 5.2 Content Data

- Public vows and comments are retained permanently as part of the service's commitment to accountability
- Private vows may be retained for a reasonable period after account deletion

### 5.3 Automatic Deletion

- We periodically review and delete inactive accounts and associated data
- Users can request deletion of their data subject to legal requirements

## 6. Your Rights and Choices

### 6.1 Access and Control

- You can access and update your account information
- You can change vow visibility settings
- You can delete comments and private vows

### 6.2 Data Portability

- You can export your vow data in a machine-readable format
- You can request a copy of your personal information

### 6.3 Data Deletion

- You can delete your account and associated data
- Some public content may remain accessible due to the nature of public commitments
- We will retain minimal data for legal compliance

### 6.4 Opt-Out Rights

- You can opt out of non-essential communications
- You can disable cookies through browser settings
- You can request restrictions on data processing

## 7. Data Security

### 7.1 Security Measures

- We use industry-standard encryption for data transmission
- We implement appropriate technical and organizational measures
- We regularly review and update our security practices

### 7.2 Data Breaches

- In the event of a security breach, we will notify affected users
- We will take appropriate measures to mitigate harm
- We will comply with applicable breach notification laws

## 8. International Data Transfers

- Your information may be transferred to and processed in countries other than your own
- We ensure appropriate safeguards are in place for international transfers
- We comply with applicable data protection laws

## 9. Children's Privacy

- Our service is not intended for children under 13
- We do not knowingly collect personal information from children under 13
- If we become aware of such collection, we will delete the information

## 10. Third-Party Links and Services

- Our service may contain links to third-party websites
- We are not responsible for the privacy practices of third parties
- We encourage you to review the privacy policies of any third-party services

## 11. Cookies and Tracking Technologies

### 11.1 Types of Cookies

- **Essential Cookies:** Required for basic service functionality (authentication, session management)
- **Preference Cookies:** Remember your settings and preferences (theme selection)

### 11.2 Cookie Management

- You can control cookies through your browser settings
- Disabling cookies may affect service functionality
- We do not use cookies for advertising purposes

## 12. Changes to This Privacy Policy

- We may update this Privacy Policy from time to time
- We will notify users of material changes
- Continued use of the service constitutes acceptance of updated policy
- We encourage regular review of this policy

## 13. Contact Us

If you have questions about this Privacy Policy or our data practices, please contact us at:

**Email:** [privacy@vowed.com]  
**Address:** [Your Business Address]  
**Phone:** [Your Phone Number]

## 14. Data Protection Officer

For EU users, our Data Protection Officer can be contacted at [dpo@vowed.com]

## 15. Complaints

If you believe we have not complied with applicable data protection laws, you have the right to lodge a complaint with the relevant supervisory authority.
